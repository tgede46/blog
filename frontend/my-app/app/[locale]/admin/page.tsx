  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="flex flex-wrap items-end justify-between gap-5">
        <div><p className="text-sm font-semibold text-violet-700">{t("dashboardLabel")}</p><h1 className="mt-1 font-heading text-4xl font-black">{t("dashboard")}</h1></div>
      ...
      <section className="neo-card mt-8 overflow-hidden">
        <div className="flex items-center justify-between border-b-2 border-on-surface p-5"><h2 className="font-heading text-xl font-bold">{t("recentPosts")}</h2><Link href="/admin/posts" className="text-sm font-bold text-violet-700">{t("manageAll")} →</Link></div>
        {posts.length ? <div className="divide-y divide-black/10">{posts.map((post) => <div key={post.id} className="flex flex-wrap items-center justify-between gap-4 p-5"><div><p className="font-bold">{post.title}</p><p className="mt-1 text-sm text-[#737b8d]">{formatDateFr(post.updated_at || post.date)} · {post.status === "published" ? tCommon("published") : tCommon("draft")}</p></div><Link className="text-sm font-bold text-violet-700" href={`/admin/posts/${post.id}/edit`}>{tCommon("edit")}</Link></div>)}</div> : <p className="p-8 text-center text-[#737b8d]">{t("noPosts")}</p>}
      </section>
