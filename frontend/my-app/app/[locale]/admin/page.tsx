...
export default function AdminDashboardPage() {
  ...
  const cards = [
    { label: t("views"), value: stats?.total_views, icon: Eye, color: "bg-[#e8e2fa]" },
    { label: t("posts"), value: stats ? stats.published + stats.drafts : undefined, icon: FileText, color: "bg-[#dff3e5]" },
    { label: t("users"), value: filteredUsers, icon: Users, color: "bg-[#f8e7b6]" },
  ]

  const audience = [
    { key: "admin" as const, label: t("admins"), value: stats?.admins, icon: Shield, hint: t("adminsHint") },
    { key: "editor" as const, label: t("editors"), value: stats?.editors, icon: UserCog, hint: t("editorsHint") },
    { key: "subscriber" as const, label: t("subscribers"), value: stats?.subscribers, icon: Users, hint: t("subscribersHint") },
  ]
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="flex flex-wrap items-end justify-between gap-5">
        <div><p className="text-sm font-semibold text-on-surface">{t("dashboardLabel")}</p><h1 className="mt-1 font-heading text-4xl font-black">{t("dashboard")}</h1></div>
        <Link href="/admin/new" className="neo-button bg-tertiary-fixed px-5 py-3 font-black uppercase tracking-wide">{t("newPost")}</Link>
      </div>
      ...
      <section className="neo-card mt-8 p-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h2 className="font-heading text-xl font-bold">{t("platformUsers")}</h2>
        ...
        <div className="mt-6 flex flex-wrap gap-3" role="group" aria-label={t("filterUsers")}>
      ...
      <section className="neo-card mt-8 overflow-hidden">
        <div className="flex items-center justify-between border-b-2 border-on-surface p-5"><h2 className="font-heading text-xl font-bold">{t("recentPosts")}</h2><Link href="/admin/posts" className="text-sm font-bold text-on-surface">{t("manageAll")} →</Link></div>
        {posts.length ? <div className="divide-y divide-black/10">{posts.map((post) => <div key={post.id} className="flex flex-wrap items-center justify-between gap-4 p-5"><div><p className="font-bold">{post.title}</p><p className="mt-1 text-sm text-[#737b8d]">{formatDateFr(post.updated_at || post.date)} · {post.status === "published" ? tCommon("published") : tCommon("draft")}</p></div><Link className="text-sm font-bold text-on-surface" href={`/admin/posts/${post.id}/edit`}>{tCommon("edit")}</Link></div>)}</div> : <p className="p-8 text-center text-[#737b8d]">{t("noPosts")}</p>}
      </section>
      {stats?.activity.length ? <section className="neo-card mt-8 p-6"><h2 className="font-heading text-xl font-bold">{t("recentActivity")}</h2><ul className="mt-5 space-y-4">{stats.activity.map((item, index) => <li key={`${item.type}-${item.created_at}-${index}`} className="border-l-4 border-tertiary-fixed pl-4"><p className="font-semibold">{item.label}</p><p className="text-sm text-[#737b8d]">{formatDateFr(item.created_at)}</p></li>)}</ul></section> : null}
    </div>
  )
}
