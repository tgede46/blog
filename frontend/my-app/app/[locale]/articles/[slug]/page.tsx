...
function readingLabel(value?: string | number, locale?: string) {
  const label = readingTime(value)
  const match = String(label).match(/(\d+)/)
  const unit = locale === "en" ? "MIN READ" : locale === "es" ? "MIN DE LECTURA" : "MN DE LECTURE"
  return match ? `${match[1]}${unit}` : "LECTURE RAPIDE"
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params
  const source = await article(slug).catch(() => null)
  return source
    ? {
        title: source.title,
...
export default async function ArticlePage({ params }: Props) {
  ...
  const source = await article(slug)
  if (!source) notFound()
      ...
      <main id="contenu" className="mx-auto max-w-3xl px-5 py-14 sm:py-20">
        <header>
          ...
          <p className="mt-5 text-xs font-bold uppercase tracking-[0.18em] text-[#6b7280]">
            {readingLabel(source.minutes, locale)}
          </p>
        </header>

        {headings.length > 0 && (
          <nav className="mt-10 border-l-2 border-[#1d2433]/15 pl-4 hidden sm:block" aria-label={t("tableOfContents")}>
            <p className="mb-3 text-xs font-bold uppercase tracking-wider text-[#6b7280]">{t("tableOfContents")}</p>
        ...
        <article className="mt-12 sm:mt-16">
          ...
          <div className="mt-16 border-t border-[#1d2433]/10 pt-8">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#6b7280]">{t("writtenBy")}</p>
            ...
            <p className="mt-3 max-w-xl leading-7 text-[#596275]">
              {String(settings.author_bio || tArticle("defaultExcerpt"))}
            </p>
          </div>
        </article>
        ...
        {related.length > 0 && (
          <section className="mt-20">
            <h2 className="mb-8 font-heading text-3xl font-black">{t("readNext")}</h2>
