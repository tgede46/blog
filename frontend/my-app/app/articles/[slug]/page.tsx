...
export default async function ArticlePage({ params }: Props) {
  ...
  return (
    <div className="graphic-grid min-h-screen bg-surface text-on-surface"><Nav /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replace(/</g, "\\u003c") }} /><main id="contenu" className="mx-auto max-w-6xl px-5 py-14">
      <header className="mx-auto max-w-4xl text-center"><div className="flex justify-center gap-3 text-xs font-bold uppercase"><span className="neo-button -rotate-1 bg-node-green px-4 py-2">{source.category}</span><span className="neo-button rotate-1 bg-ts-blue px-4 py-2">{source.tag}</span></div><h1 className="mt-9 font-heading text-4xl font-black leading-tight sm:text-6xl">{source.title}</h1><p className="mx-auto mt-6 max-w-2xl text-lg leading-8 text-[#687184]">{source.excerpt}</p><p className="mt-5 text-sm font-bold uppercase tracking-wider text-[#858b98]">{formatDateFr(source.published_at || source.date)} · {readingTime(source.minutes)}</p></header>
      ...
      <article className="neo-card min-w-0 p-6 sm:p-10"><div className="space-y-6 text-[1.08rem] leading-8 text-[#343b4a]">{source.intro && <p className="text-xl font-medium leading-9">{source.intro}</p>}{source.content.map((block, index) => {
        ...
        if (block.type === "callout") return <blockquote key={index} className="border-2 border-on-surface border-l-8 bg-[#eeeafb] p-6 hard-shadow-sm">{block.text}</blockquote>
        if (block.type === "image" && (block.url || block.text)) {
          const imageUrl = block.url || block.text || ""
          return <figure key={index}><img src={imageUrl} alt={block.alt || block.caption || ""} className="w-full border-2 border-on-surface hard-shadow" /><figcaption className="mt-3 text-center text-sm">{block.caption}</figcaption></figure>
        }
        return null
      })}</div><div className="mt-14 border-2 border-on-surface bg-tertiary-fixed p-7 hard-shadow-sm"><p className="text-xs font-bold uppercase text-violet-700">Écrit par</p><h2 className="mt-2 font-heading text-2xl font-bold">{String(settings.author_name || "Gedeon Kpara")}</h2><p className="mt-3 text-[#687184]">{String(settings.author_bio || "Développeur et auteur de ce blog.")}</p></div></article></div>
