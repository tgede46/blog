  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="flex flex-wrap items-end justify-between gap-5">
        <div><p className="text-sm font-semibold text-on-surface">Vue d’ensemble</p><h1 className="mt-1 font-heading text-4xl font-black">Tableau de bord</h1></div>
      ...
      <section className="neo-card mt-8 overflow-hidden">
        <div className="flex items-center justify-between border-b-2 border-on-surface p-5"><h2 className="font-heading text-xl font-bold">Articles récents</h2><Link href="/admin/posts" className="text-sm font-bold text-on-surface">Tout gérer →</Link></div>
        {posts.length ? <div className="divide-y divide-black/10">{posts.map((post) => <div key={post.id} className="flex flex-wrap items-center justify-between gap-4 p-5"><div><p className="font-bold">{post.title}</p><p className="mt-1 text-sm text-[#737b8d]">{formatDateFr(post.updated_at || post.date)} · {post.status === "published" ? "Publié" : "Brouillon"}</p></div><Link className="text-sm font-bold text-on-surface" href={`/admin/posts/${post.id}/edit`}>Modifier</Link></div>)}</div> : <p className="p-8 text-center text-[#737b8d]">Aucun article à afficher.</p>}
      </section>
