...
export default function AdminDashboardPage() {
  ...
  const cards = [
    ...
    { label: "Articles", value: stats?.total_posts, icon: FileText, color: "bg-[#dff3e5]" },
    { label: "Abonnés", value: stats?.total_subscribers, icon: Users, color: "bg-[#f8e7b6]" },
  ]
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      {stats?.recent_activities?.length ? <section className="mt-8 rounded-2xl border border-black/10 bg-white p-6"><h2 className="font-heading text-xl font-bold">Activité récente</h2><ul className="mt-5 space-y-4">{stats.recent_activities.map((item) => <li key={item.id} className="border-l-2 border-violet-300 pl-4"><p className="font-semibold">{item.action} — {item.title}</p><p className="text-sm text-[#737b8d]">{item.time}</p></li>)}</ul></section> : null}
    </div>
  )
}
