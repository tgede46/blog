...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (isLoading || !user) {
    return <div className="flex min-h-screen items-center justify-center bg-[#f7f6f2] font-semibold text-[#596275]">Vérification de la session…</div>
  }
  ...
  const sidebar = (
    <>
      ...
      <div className="border-t border-black/10 p-4">
        ...
        <button onClick={() => void handleLogout()} className="mt-3 flex w-full items-center gap-3 rounded-xl px-4 py-3 font-semibold text-red-700 hover:bg-red-50"><LogOut size={18} />Déconnexion</button>
      </div>
    </>
  )

  return (
    <div className="min-h-screen bg-[#f7f6f2] text-[#1d2433] md:flex">
      <aside className="hidden min-h-screen w-64 shrink-0 flex-col border-r border-black/10 bg-white md:flex">{sidebar}</aside>
      ...
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-black/10 bg-white/95 px-5 backdrop-blur md:px-8">
