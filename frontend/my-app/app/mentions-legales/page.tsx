  ...
  return (
    <PublicPage>
      ...
      <div className="mt-10 space-y-9 text-base leading-8 text-[#596275]">
        <section><h2 className="font-heading text-2xl font-bold text-[#1d2433]">Éditeur</h2><p className="mt-2">Ce site est édité par {String(owner)}{settings.address ? `, ${String(settings.address)}` : ""}. Contact : {String(settings.contact_email || "via le formulaire de contact")}.</p></section>
        ...
        {settings.legal_content && <section><h2 className="font-heading text-2xl font-bold text-[#1d2433]">Informations complémentaires</h2><p className="mt-2 whitespace-pre-line">{settings.legal_content}</p></section>}
      </div>
    </PublicPage>
  )
}
