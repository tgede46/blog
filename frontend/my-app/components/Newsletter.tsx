...
export default function Newsletter() {
  ...
  return (
    <section id="newsletter" className="relative my-24 overflow-hidden rounded-[2rem] bg-[#1d2433] px-7 py-14 text-white sm:px-14">
      ...
      <form onSubmit={subscribe}><div className="flex flex-col gap-3 sm:flex-row"><label className="sr-only" htmlFor="newsletter-email">Votre email</label><input id="newsletter-email" value={email} onChange={(event) => setEmail(event.target.value)} type="email" required className="min-w-0 flex-1 rounded-full border border-white/20 bg-white/10 px-6 py-4 outline-none" placeholder="vous@exemple.fr" /><button disabled={loading} className="rounded-full bg-[#f5d989] px-7 py-4 font-bold text-[#1d2433]">{loading ? "Envoi…" : "S’inscrire"}</button></div><p className="mt-3 min-h-6 text-sm text-white/70" aria-live="polite">{message}</p></form></div>
    </section>
  )
}
